...
export default BreadCrumbs = {
  ...
  activate(state) {
    ...
    let trackChange = atom.workspace.observeTextEditors(editor => editor
    console.log("hiiii" + trackChange)
    //   console.log(editor)
    //   currentChange = editor.buffer.onDidChange(function () {
    //     console.log('changing status' + editor)
    //   })
    // })
    ...
    // this.commentModal = atom.workspace.addModalPanel({
    //   item: this.newComment(),
    //   visible: true
    // });
  ...
  recordOn(state) {
    ...
    this.recWorkspaces = atom.workspace.observeTextEditors(function (editor) {
      currentRecording = editor.buffer.onWillSave(function () {
        exposedUndoFilePath = path.join(editor.buffer.file.path + '.crumb')
        undoFilePath = exposedUndoFilePath.replace(/(\S+\/)([\w+\.]+)/, '$1.$2')
