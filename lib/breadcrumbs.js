...
import PlayBtnView from './playBtn-view';
...
export default BreadCrumbs = {
  ...
  activate(state) {
    ...
    this.playBtnView = new PlayBtnView
    ...
    this.playBtn = atom.workspace.addBottomPanel({
      item: this.playBtnView.getElement(),
      className: 'play-btn-panel',
      visible: true
    });
    // this.playBtn.id = 'play-btn-panel'
    this.bottomPanel = atom.workspace.addBottomPanel({
  ...
  checkoutSandbox() {
    ...
    if(repoPath !== '') {
      ...
      this.createGitNotification('sandbox', 'Have fun.', repoPath)
    } else {
  ...
  gitCheckback() {
    ...
    if(repoPath !== '') {
      ...
      this.createGitNotification('master', 'Sandbox branch has been pruned.', repoPath)
    } else {
      this.createNoGitError(repoPath);
    };
  },
  ...
  getCurrentRepoPath() {
    ...
    if(projectRepos.length === 1 && projectRepos[0] !== null) {
      return projectRepos[0].getWorkingDirectory() }
    else {
      let editor = atom.workspace.getActiveTextEditor();
